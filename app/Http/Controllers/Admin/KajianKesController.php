...
class KajianKesController extends Controller
{
    /**
    ...
    {
        $query = KajianKes::query();

        return DataTables::of($query)
            ->addColumn('action_buttons', function ($row) {
                $viewBtn = '<button class="btn btn-sm btn-primary view-kajian-btn" data-id="' . $row->id . '" title="Lihat Butiran">
                    <i class="fas fa-eye"></i>
                </button>';
                $editBtn = '<button class="btn btn-sm btn-info edit-kajian-btn" data-id="' . $row->id . '" title="Edit">
                    <i class="fas fa-edit"></i>
                </button>';
                $deleteBtn = '<button class="btn btn-sm btn-danger delete-kajian-btn" data-id="' . $row->id . '" title="Padam">
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'nama_ketua_penidik' => 'required|string|max:255',
            'ahli' => 'nullable|string',
            'tajuk_kajian_kes' => 'required|string|max:255',
    ...
    {
        $kajian = KajianKes::findOrFail($id);
        return response()->json($kajian->toArray());
    }
