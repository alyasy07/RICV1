...
class PenerbitanPenulisanController extends Controller
{
    /**
     * Display the Penerbitan dan Penulisan Kreatif page
     */
    public function index()
    {
        return view('Admin.penerbitanpenulisan');
    }
    ...
    {
        $penerbitan = Pelaporan::where('jenis', 'penerbitan_penulisan')
            ->select([
                'pelaporan.id', 
                'title as pelaporan',
                'pemberi_dana',
        ...
        if ($request->filled('date_from')) {
            $penerbitan->whereDate('tarikh_tutup', '>=', $request->date_from);
        }
        
        if ($request->filled('date_to')) {
            $penerbitan->whereDate('tarikh_tutup', '<=', $request->date_to);
        }

        return DataTables::of($penerbitan)
            ->addIndexColumn()
    ...
    public function update(Request $request, $id)
    {
        ...
        $penerbitan = PenerbitanPenulisan::where('pelaporan_id', $id)->first();
        
        if (!$penerbitan) {
            $penerbitan = new PenerbitanPenulisan();
            $penerbitan->pelaporan_id = $id;
            $penerbitan->save();
        }
    ...
    {
        // Deleting the pelaporan will cascade delete the related penerbitan
        $pelaporan = Pelaporan::findOrFail($id);
        $pelaporan->delete();
