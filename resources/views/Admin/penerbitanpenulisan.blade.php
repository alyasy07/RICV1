...
<head>
    ...
    <title>Penerbitan dan Penulisan Kreatif – {{ config('app.name') }}</title>
...
<body id="page-top">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h2 mb-0 text-gray-900 gradient-title">Penerbitan dan Penulisan Kreatif</h1>
          </div>
          ...
          <div class="card shadow-sm">
            <div class="card-body">
              <div class="table-responsive">
                <table class="table table-bordered table-hover table-striped" id="penerbitanTable" width="100%" cellspacing="0">
          ...
          <!-- Add/Edit Modal -->
          <div class="modal fade" id="penerbitanModal" tabindex="-1" role="dialog" aria-labelledby="penerbitanModalLabel">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-primary text-white">
                        <h5 class="modal-title" id="penerbitanModalLabel">Tambah Penerbitan & Penulisan Kreatif</h5>
                        <button type="button" class="close text-white" data-dismiss="modal" aria-label="Tutup">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form id="penerbitanForm">
                        @csrf
                        <input type="hidden" id="penerbitan_id">
            ...
            <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                <div class="modal-header bg-info text-white">
                  <h5 class="modal-title" id="viewDetailsModalLabel">Butiran Penerbitan & Penulisan Kreatif</h5>
                  <button type="button" class="close text-white" data-dismiss="modal" aria-label="Tutup">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      var table = $('#penerbitanTable').DataTable({
        responsive: true,
        processing: true,
        serverSide: true,
        pageLength: 25,
        ...
        order: [[1, 'asc']], // Default sort by Tajuk
        initComplete: function() {
            ...
            $('.dataTables_filter').append(
                ...
                '<a href="#" class="btn btn-primary ml-2" data-toggle="modal" data-target="#penerbitanModal" style="background:#123cc5ff;border-color:#123cc5ff;color:#fff;">' +
                '<i class="fas fa-plus"></i> Tambah Penerbitan</a>'
            );
        },
        ajax: {
          url: '/admin/penerbitan-penulisan/data',
          data: function(d) {
      ...
      $('#penerbitanForm').on('submit', function(e) {
        e.preventDefault();
        
        var formData = $(this).serialize();
        var id = $('#penerbitan_id').val();
        var url = id ? '/admin/penerbitan-penulisan/' + id : '/admin/penerbitan-penulisan';
        ...
        $.ajax({
          ...
          success: function(result) {
            $('#penerbitanModal').modal('hide');
            $('#penerbitanForm')[0].reset();
            $('#penerbitan_id').val('');
      ...
      $(document).on('click', '.view-btn', function() {
        ...
        $.ajax({
          url: '/admin/penerbitan-penulisan/' + id + '/edit',
          type: 'GET',
      ...
      $(document).on('click', '.edit-btn', function() {
        ...
        $.ajax({
          ...
          type: 'GET',
          success: function(data) {
            $('#penerbitanModalLabel').text('Edit Penerbitan & Penulisan Kreatif');
            $('#penerbitan_id').val(data.id);
      ...
      $('#confirmDelete').on('click', function() {
        ...
        $.ajax({
          url: '/admin/penerbitan-penulisan/' + id,
          type: 'DELETE',
          data: {
            '_token': '{{ csrf_token() }}'
          },
      ...
      $('#penerbitanModal').on('hidden.bs.modal', function() {
        $('#penerbitanModalLabel').text('Tambah Penerbitan & Penulisan Kreatif');
        $('#penerbitanForm')[0].reset();
        $('#penerbitan_id').val('');
      });
